...
def freezer(ice_cream_shop_id):
    ...
    for item in inventory:
        ...
        if flavor:
            ...
            print(f"Flavor ID: {item.flavor_id}")  # added print statement
...
def add_to_freezer(ice_cream_shop_id, flavor_id):
    ...
    if request.method == 'POST':
        ...
        if flavor_id and quantity:
            ...
            flash(f'{quantity} units of flavor {Flavor.get_one_by_id(flavor_id).name} have been added to the freezer at {ice_cream_shop.name}.', 'success')
            return redirect(url_for('ice_cream_shops', ice_cream_shop_id=ice_cream_shop_id))
        else:
