...
def add_to_freezer(ice_cream_shop_id):
    ...
    if not ice_cream_shop:
        flash('Ice cream shop not found', 'error')
        return redirect(url_for('ice_cream_shops'))

    # Check if the user owns the ice cream shop
    if ice_cream_shop.user_id != user.id:
        flash('You are not authorized to add flavors to this ice cream shop.', 'error')
        return redirect(url_for('dashboard'))

    # Get flavor information from the form data
    name = request.form['name']
    ...
    if freezer_inventory:
        freezer_inventory.quantity += quantity
    ...
    return redirect(url_for('ice_cream_shop', ice_cream_shop_id=ice_cream_shop_id))