...
def add_to_freezer(flavor_id):
    ...
    if request.method == 'POST':
        ...
        freezer = FreezerInventory({'shop_id': shop_id, 'flavor_id': flavor_id, 'quantity': quantity})
        freezer.save()
        flash('Successfully added to freezer!', 'success')
        return redirect(f'/ice_cream_shops/{shop_id}')
    ice_cream_shops = IceCreamShop.query.all()
