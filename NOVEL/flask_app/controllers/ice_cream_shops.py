...
def freezer(ice_cream_shop_id):
    ...
    for item in inventory:
        ...
        if flavor:
            item_data = {
            'flavor_id': item.flavor_id,  # Change this line
            'name': flavor.name,
    ...
    return render_template('freezer.html', ice_cream_shop=ice_cream_shop, items=items, ice_cream_shop_id=ice_cream_shop_id, ice_cream_flavors=current_flavors['ice_cream_flavors'], sorbet_flavors=current_flavors['sorbet_flavors'])
...
def update_freezer(ice_cream_shop_id, flavor_id, action=None):
    ...
    if user.id != IceCreamShop.get_one_by_id(ice_cream_shop_id).user_id:  # Check if user ID matches shop ID
