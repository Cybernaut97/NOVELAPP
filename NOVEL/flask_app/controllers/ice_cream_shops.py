...
@app.route('/ice_cream_shops/<int:shop_id>/freezer/add', methods=['GET', 'POST'])
def add_to_freezer(shop_id):
    ice_cream_shop = IceCreamShop.get_one_by_id({'id': shop_id})
    if request.method == 'POST':
        flavor_id = request.form['flavor_id']
        quantity = request.form['quantity']
        freezer_inventory = FreezerInventory.get_by_shop_and_flavor(shop_id, flavor_id)
        ...
        return redirect(url_for('freezer', ice_cream_shop_id=shop_id))
    else:
