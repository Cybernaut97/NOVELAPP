...
def add_to_freezer(flavor_id, shop_id):
    ...
    if request.method == 'POST':
        ...
        if not quantity:
            ...
            return redirect(f'/add_to_freezer/{flavor_id}/{shop_id}')
        shop = IceCreamShop.get_one_by_id(shop_id)
        if not shop:
            flash('Ice cream shop not found')
            return redirect('/')
        freezer = FreezerInventory.get_one_by_shop_id_and_flavor_id(shop_id, flavor_id)
        if freezer:
            ...
            freezer = FreezerInventory(shop_id=shop_id, flavor_id=flavor_id, quantity=int(quantity))
            freezer.save()
            flash(f'{quantity} pans of {flavor.name} added to freezer inventory for ice cream shop {shop.name}')
        return redirect(f'/add_to_freezer/{flavor_id}/{shop_id}')
    else:
        ice_cream_shops = IceCreamShop.get_all()
        return render_template('add_to_freezer.html', flavor=flavor, ice_cream_shops=ice_cream_shops)
