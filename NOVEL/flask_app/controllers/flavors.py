...
def edit_flavor():
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('flavors', ice_cream_shop_id=11))

    flavors = Flavor.get_all()
    return render_template('edit_flavor.html', flavors=flavors)
...
@app.route("/delete/<int:flavor_id>/delete/<int:ice_cream_shop_id>", methods=["POST"])
def delete_flavor(flavor_id, ice_cream_shop_id):
    if 'id' not in session:
        return redirect('/')
