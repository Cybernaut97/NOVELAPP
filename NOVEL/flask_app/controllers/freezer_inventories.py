...
@app.route('/freezer_inventories/<int:inventory_id>/set_current', methods=['POST'])
def set_current_flavor(inventory_id):
    inventory = FreezerInventory.get_one_by_id(inventory_id)
    if inventory:
        inventory.current_flavor = True
        inventory.quantity -= 1
        inventory.save({'current_flavor': inventory.current_flavor, 'quantity': inventory.quantity})
        flash(f'{inventory.flavor.name} set as current flavor and removed 1 from the quantity!', 'success')
    else:
        flash('Inventory item not found', 'error')

    return redirect(url_for('freezer', ice_cream_shop_id=inventory.shop_id))