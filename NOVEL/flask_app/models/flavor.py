...
class Flavor:
    def __init__(self, data):
        ...
        self.is_staple = data.get('is_staple', False)

    @classmethod
    def update(cls, data):
        query = """UPDATE flavors SET name=%(name)s, description=%(description)s, is_ice_cream=%(is_ice_cream)s, is_staple=%(is_staple)s WHERE id=%(id)s"""
    ...
    def save(cls, data=None):
        query = """INSERT INTO flavors (name, description, is_ice_cream, is_staple)  VALUES (%(name)s, %(description)s, %(is_ice_cream)s, %(is_staple)s);"""
        return connectToMySQL(DATABASE).query_db(query, data)
    ...
    @classmethod
    def get_staple_flavors(cls):
        query = "SELECT * FROM flavors WHERE is_staple = 1"
        results = connectToMySQL(DATABASE).query_db(query)
        staple_flavors = []
        for flavor in results:
            staple_flavors.append(cls(flavor))
        return staple_flavors
