...
class IceCreamShop:
    ...
    @classmethod
    def save(cls, data):
        query = "INSERT INTO ice_cream_shops (name, address, phone_number, square_location_id, created_at, updated_at, user_id) VALUES (%(name)s, %(address)s, %(phone_number)s, %(square_location_id)s, NOW(), NOW(), %(user_id)s);"
    ...
    @classmethod
    def edit(cls, data):
        query = "UPDATE ice_cream_shops SET name = %(name)s, address = %(address)s, phone_number = %(phone_number)s, square_location_id = %(square_location_id)s, updated_at = NOW() WHERE id = %(id)s;"
        return connectToMySQL(DATABASE).query_db(query, data)

    @classmethod
    def update(cls, data):
        query = "UPDATE ice_cream_shops SET name = %(name)s, address = %(address)s, phone_number = %(phone_number)s, square_location_id = %(square_location_id)s WHERE id = %(id)s;"
        return connectToMySQL(DATABASE).query_db(query, data)
    ...
    def get_one_by_id(cls, id):
        ...
        if not results:
            print(f"No ice cream shop found with id {id}")
            return None
        else:
    ...
    @classmethod
    def assign_user(cls, shop_id, user_id):
        query = "UPDATE ice_cream_shops SET user_id = %(user_id)s WHERE id = %(shop_id)s"
        data = {'user_id': user_id, 'shop_id': shop_id}
        return connectToMySQL(DATABASE).query_db(query, data)

    @classmethod
    def remove_user(cls, shop_id):
        query = "UPDATE ice_cream_shops SET user_id = NULL WHERE id = %(shop_id)s"
